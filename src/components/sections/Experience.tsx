...
function RoleCard({ role, company, roleIndex }: { role: typeof EXPERIENCE[0]["roles"][0]; company: string; roleIndex: number }) {
  ...
  const hasMore = role.responsibilities.length > VISIBLE_COUNT;
  const visibleItems = expanded ? role.responsibilities : role.responsibilities.slice(0, VISIBLE_COUNT);
            ...
            <div>
              ...
              {hasMore && (
                <button
                  ...
                  className="mt-4 flex items-center gap-1.5 text-sm font-medium text-accent hover:text-accent/80 transition-colors"
                ...
                >
                  {expanded ? (
                    ...
                    <>
                      ...
                      View More ({role.responsibilities.length - VISIBLE_COUNT})
                    </>
                  )}
                </button>
              )}
